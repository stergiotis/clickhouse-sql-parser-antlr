...
queryStmt: (query (INTO OUTFILE STRING_LITERAL)? (FORMAT identifierOrNull)? (SEMICOLON)? | insertStmt) EOF;
query:
	alterStmt // DDL
	| attachStmt // DDL
	| checkStmt
...
alterStmt: ALTER TABLE tableIdentifier clusterClause? alterTableClause (COMMA alterTableClause)* # AlterTableStmt;
...
// ATTACH statement
attachStmt: ATTACH DICTIONARY tableIdentifier clusterClause? # AttachDictionaryStmt;
...
createStmt: (ATTACH | CREATE) DATABASE (IF NOT EXISTS)? databaseIdentifier clusterClause? engineExpr?																	# CreateDatabaseStmt
...
explainStmt: EXPLAIN AST query # ExplainASTStmt | EXPLAIN SYNTAX query # ExplainSyntaxStmt;
...
projectionSelectStmt: LPAREN withClause? SELECT columnExprList groupByClause? projectionOrderByClause? RPAREN;
...
joinOp: ((ALL | ANY | ASOF)? INNER | INNER (ALL | ANY | ASOF)? | (ALL | ANY | ASOF))	# JoinOpInner
	| (
...
joinConstraintClause: ON columnExprList | USING LPAREN columnExprList RPAREN | USING columnExprList;
...
winFrameExtend: winFrameBound # frameStart | BETWEEN winFrameBound AND winFrameBound # frameBetween;
winFrameBound: (
		CURRENT ROW
		| UNBOUNDED PRECEDING
		| UNBOUNDED FOLLOWING
...
columnLambdaExpr: ( LPAREN identifier (COMMA identifier)* RPAREN | identifier (COMMA identifier)*) ARROW columnExpr;
